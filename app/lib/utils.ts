...
export function sliceText(text: string, maxNameLength: number = 30, abbr: true | false = true, plain: boolean = true) {
  if (abbr) {
    return text.length > maxNameLength ? `<abbr title="${text}">${text.slice(0, maxNameLength - 3)}...</abbr>` : text;
  } else {
    if (plain) {
      return text.length > maxNameLength ? `${text.slice(0, maxNameLength - 3)}...` : text;
    }
    return text.length > maxNameLength ? `<abbr title="more">${text.slice(0, maxNameLength - 3)}...</abbr>` : text;
  }
}

export function splitAndGetLast(text: string, seperator: string, maxLength: number = 15) {
